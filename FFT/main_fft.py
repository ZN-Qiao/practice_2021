...
parser.add_argument('data', metavar='DIR', default='/home/zhinan/data/places',
...
parser.add_argument('--multiprocessing-distributed', action='store_true',
                    help='Use multi-processing distributed training to launch '
                         ...
                         'multi node data palllel training')
parser.add_argument('--bw', default=3.0, type=float, help='bandwidth')
parser.add_argument('--filter', default=0, type=int, help='filter')
...
def main():
    ...
    if args.filter == 0:
        filer = LowPass(224, args.bw, False)
        print('low', args.bw)
    elif args.filter == 1:
        filer = HighPass(224, args.bw, False)
        print('high', args.bw)
    ...
    if args.multiprocessing_distributed:
        ...
        main_worker(args.gpu, ngpus_per_node, args, filer)
...
def main_worker(gpu, ngpus_per_node, args, filter):
    global best_acc1
    args.gpu = gpu
    ...
    normalize = transforms.Normalize(mean=[0.408, 0.441, 0.458],
                                     std=[0.271, 0.256, 0.260])
    ...
    if args.evaluate:
        validate(val_loader, model, criterion, args, filter)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1 = validate(val_loader, model, criterion, args, filter)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    record_str = str(epoch) + '\t' + "%.3f" % (losses.avg) + '\t' + "%.3f" % (top1.avg) + '\t' + "%.3f" % (
        top5.avg) + '\n'
    write_record(file_path, record_str)


def validate(val_loader, model, criterion, args, filter):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
