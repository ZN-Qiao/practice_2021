...
parser.add_argument('--weight1', default=1.0, type=float,  help='weight1')
parser.add_argument('--weight2', default=0.5, type=float, help='weight2')
...
try:
    net = models.__dict__[args.netName](num_classes=args.cifar, weight=[args.weight1, args.weight2])
except:
...
def train(epoch):
    ...
    file_path='records/cifar_' + str(args.cifar) + '_' + args.netName + '_' + str(args.weight1) + str(args.weight2) + '_train.txt'
...
def test(epoch):
    ...
    file_path = 'records/cifar_' + str(args.cifar) + '_' + args.netName + '_' + str(args.weight1)  + str(args.weight2) + '_test.txt'
    ...
    if acc > best_acc:
        ...
        # save_path = './checkpoint/cifar'+str(args.cifar)+"/"+args.netName+"/"+str(args.weight1)  + str(args.weight2) +"_model_best.t7"
        # torch.save(state, save_path)
        best_acc = acc

    return [(test_loss / (batch_idx + 1)), (100. * correct / total)]

logfile = './checkpoint/cifar'+str(args.cifar)+"/"+args.netName+"/"+str(args.weight1)+  str(args.weight2)  +  ".txt"
